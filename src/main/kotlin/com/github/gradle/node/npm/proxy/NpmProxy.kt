...
object NpmProxy {
    ...
    private val proxyVariables: List<String> = listOf(
        "HTTP_PROXY",
        "HTTPS_PROXY",
        "NO_PROXY",
        "PROXY",
    ...
    private val npmProxyVariables: List<String> = listOf(
        "NPM_CONFIG_PROXY",
    ...
    fun shouldConfigureProxy(env: Map<String, String>, settings: ProxySettings): Boolean {
        return when (settings) {
            ProxySettings.FORCED -> true
            ProxySettings.SMART -> !hasProxyConfiguration(env)
            ProxySettings.OFF -> false
        }
    }
